...
class App(tk.Tk):
    def __init__(self):
        ...
        self.create_frame("Oppervlakte berekening", self.calculate_oppervlakte, "Voer de lengte in (m):", "Voer de breedte in (m):") #label voor vierkant meter berekening, gemaakt door Musabe
    ...
    def calculate_oppervlakte(self, entry1, entry2):
        lengte = float(entry1.get())
        breedte = float(entry2.get())
        omtrek = 2 * (lengte + breedte)
        oppervlakte = lengte * breedte
        self.result(omtrek, "Omtrek", str(oppervlakte), "Oppervlakte")
